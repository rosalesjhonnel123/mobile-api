...
class Url {
    ...
    // Check for ajax
		if( $type == 'XMLHttpRequest' )
		  $url->method = isset($_SERVER['HTTP_X_REQUESTED_WITH']) ? $_SERVER['HTTP_X_REQUESTED_WITH'] : 'GET';	
		  		
		if( static::$route_found || (!preg_match('@^'.$route.'(?:\.(\w+))?$@uD', $url->uri, $matches) || $url->method != $type) ) {
			return false;
		}
    
    // Get the extension
    $extension = $matches[count($matches)-1];
    $extension_test = substr($url->uri, -(strlen($extension)+1), (strlen($extension)+1));
	...
	public function format($name, $callback) {
	  $url = static::instance();
	  if( !empty($url->format) && $name == $url->format )
	    echo $callback($url);
	  else
	    return false;
	}
